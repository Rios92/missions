...
if(VAS_preload && {!isNil "VAS_Local_pre_weapons"} && {!isNil "VAS_Local_pre_magazines"} && {!isNil "VAS_Local_pre_items"} && {!isNil "VAS_Local_pre_backpacks"} && {!isNil "VAS_Local_pre_glasses"}) exitWith {};

switch(_cfg) do {
	case "CfgWeapons": {
		if(!isNil {GVAR_UINS "VASP_weapons"}) exitWith {["CfgWeapons"] call VAS_Local_fnc_VASP};
		VAS_Local_pre_items = [];
		VAS_Local_pre_weapons = [];
		_temp = [];
		_ret = [];
		_ret2 = [];
		...
		
		{
			...
			_details = [_class,_cfg] call VAS_Local_fnc_fetchCfgDetails;
			...
			_str = [_class,4] call VAS_Local_fnc_KRON_StrLeft;
				
			if(_scope >= 2 && !(EQUAL(_str,"ACRE"))) then {
				switch (true) do {
					case (_type in [1,2,4,5,4096]): {
						if(_picture != "" && !(EQUAL(_displayName,""))) then {
							if((EQUAL(_type,4096) && EQUAL(_itemInfo,616)) || _AGMItem) then {
								if(_class in VAS_r_items) exitWith {}; //Exit the loop.
								VAS_Local_pre_items pushBack _class;
								pushBackMNS("VAS_Local_pre_items_misc",_class)
							} else {
								if(!(_displayName in _temp) && !(_base in VAS_r_weapons) && !(_class in VAS_r_weapons)) then {
									_temp pushBack format["%1_%2", _displayName, _AGMItem];
									VAS_Local_pre_weapons pushBack _class;
									
									//Cache filtered weapons
									switch(_type) do {
										case 1: {pushBackMNS("VAS_Local_pre_weapons_rifles",_class)};
										case 5: {pushBackMNS("VAS_Local_pre_weapons_heavy",_class)};
										case 4: {pushBackMNS("VAS_Local_pre_weapons_launchers",_class)};
										case 2: {pushBackMNS("VAS_Local_pre_weapons_pistols",_class)};
									};
								};
							};
						};
					...
					case (EQUAL(_type,131072)): {
						if(_picture != "" && !(_base in VAS_r_items) && !(_class in VAS_r_items)) then {
							...
							switch(true) do {
								case (EQUAL(_itemInfo,801)): {pushBackMNS("VAS_Local_pre_items_uniforms",_class)};
								case (EQUAL(_itemInfo,701)): {pushBackMNS("VAS_Local_pre_items_vests",_class)};
								case (EQUAL(_itemInfo,605)): {pushBackMNS("VAS_Local_pre_items_headgear",_class)};
								case (_itemInfo in [201,101,301,302]): {pushBackMNS("VAS_Local_pre_items_attachments",_class)};
								default {pushBackMNS("VAS_Local_pre_items_misc",_class)};
							};
						};
					};
				};
	...
	case "CfgMagazines":
	{
		if(!isNil {uiNamespace getVariable "VASP_magazines"}) exitWith {["CfgMagazines"] call VAS_Local_fnc_VASP;};
		...
		VAS_Local_pre_magazines = _ret;
	};
	
	case "CfgVehicles":
	{
		if(!isNil {uiNamespace getVariable "VASP_backpacks"}) exitWith {["CfgVehicles"] call VAS_Local_fnc_VASP;};
		...
		VAS_Local_pre_backpacks = _ret;
	};
	
	case "CfgGlasses":
	{
		if(!isNil {uiNamespace getVariable "VASP_glasses"}) exitWith {["CfgGlasses"] call VAS_Local_fnc_VASP;};
