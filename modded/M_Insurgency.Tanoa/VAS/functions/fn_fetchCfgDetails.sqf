/*
	...
	USAGE:
	[Classname,Cfg* (Optional)] call VAS_Local_fnc_fetchCfgDetails;
...
switch (_cfg) do
{
	...
	case "CfgWeapons":
	{
		...
		_desc = getText(_config >> "descriptionshort");
		if((isClass (_config >> "Armory")) || {configName inheritsFrom _config == "AGM_ItemCore"} || {configName inheritsFrom _config == "Item_Base_F"}) then {_AGMItem = true;};
