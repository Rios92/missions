...
switch(_cfg) do
{
	case "CfgWeapons":
	{
		...
		if(count _arr == 0) exitWith {hint "The VASP weapons config is broke?"; uiNamespace setVariable["VASP_weapons",nil]; ["CfgWeapons"] spawn VAS_Local_fnc_buildConfig;}; //Safety check
		...
		VAS_Local_pre_weapons = _weapons;
		VAS_Local_pre_items = _items;
	};
	
	case "CfgMagazines":
	{
		_arr = uiNamespace getVariable["VASP_magazines",[]];
		if(count _arr == 0) exitWith {hint "The VASP magazines config is broke?"; uiNamespace setVariable["VASP_magazines",nil]; ["CfgMagazines"] spawn VAS_Local_fnc_buildConfig;}; //Safety check
		...
		VAS_Local_pre_magazines = _magazines;
	};
	
	case "CfgVehicles":
	{
		_arr = uiNamespace getVariable["VASP_backpacks",[]];
		if(count _arr == 0) exitWith {hint "The VASP backpacks config is broke?"; uiNamespace setVariable["VASP_backpacks",nil]; ["CfgVehicles"] spawn VAS_Local_fnc_buildConfig;}; //Safety check
		...
		VAS_Local_pre_backpacks = _backpacks;
	};
	
	case "CfgGlasses":
	{
		_arr = uiNamespace getVariable["VASP_glasses",[]];
		if(count _arr == 0) exitWith {hint "The VASP glasses config is broke?"; uiNamespace setVariable["VASP_glasses",nil]; ["CfgGlasses"] spawn VAS_Local_fnc_buildConfig;}; //Safety check
		...
		VAS_Local_pre_glasses = _glasses;
	};
};