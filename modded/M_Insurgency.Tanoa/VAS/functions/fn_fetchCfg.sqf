...
switch(_request) do {
	case "guns": {
		...
		if(count VAS_weapons > 0) then {
			if(!isNil "_filter") then {
				_list = [VAS_weapons,_filter] call VAS_Local_fnc_filter;
			} else{
				_list = VAS_weapons;
			};
		} else {
			if(isNil "VAS_Local_pre_weapons") then {["CfgWeapons"] call VAS_Local_fnc_buildConfig;};
			if(!isNil "_filter") then {
				private "_cacheVar";
				_cacheVar = switch(_filter) do {case 1: {"rifles"}; case 5: {"heavy"}; case 4: {"launchers"}; case 2: {"pistols"};};
				_list = GVAR_MNS [format["VAS_Local_pre_weapons_%1", _cacheVar], []];
			} else {
				_list = VAS_Local_pre_weapons;
			};
		};
	};
	...
	case "mags": {
		...
		if(count VAS_magazines > 0) then {
			...
			if(isNil {VAS_Local_pre_magazines}) then {["CfgMagazines"] call VAS_Local_fnc_buildConfig;};
			_list = VAS_Local_pre_magazines;
		};
	};
	...
	case "items": {
		...
		if(count VAS_items > 0) then {
			if(!isNil "_filter") then {
				_list = [VAS_items,_filter] call VAS_Local_fnc_filter;
			} else {
				_list = VAS_items;
			};
		} else {
			if(isNil {VAS_Local_pre_items}) then {["CfgWeapons"] call VAS_Local_fnc_buildConfig;};
			if(!isNil "_filter") then {
				private "_cacheVar";
				...
				_list = GVAR_MNS [format["VAS_Local_pre_items_%1", _cacheVar], []];
			} else {
				_list = VAS_Local_pre_items;
			};
		};
	};
	...
	case "packs": {
		...
		{
			if(isNil {VAS_Local_pre_backpacks}) then {["CfgVehicles"] call VAS_Local_fnc_buildConfig;};
			_list = VAS_Local_pre_backpacks;
		};
	};
	...
	case "glass": {
		...
		if(count VAS_glasses > 0) then {
			...
			if(isNil {VAS_Local_pre_glasses}) then {["CfgGlasses"] call VAS_Local_fnc_buildConfig;};
			_list = VAS_Local_pre_glasses;
		};
	};
};
