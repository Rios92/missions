...
_mkr setmarkerAlpha _mAN;
                                //IF MARKER IS GREEN DO NOT CHANGE COLOUR
...
// SPAWN HOUSE PATROLS
    for "_counter" from 1 to _aGrps do {
        ...
        if (_aMin > 0) then {
            ...
            if (_debug) then {
                PLAYER SIDECHAT (format ["Spawned House Patrol: %1",_counter]);0= [_mkr,_counter,"House Patrol",getpos (leader _aGroup)] call EOS_debug
            };
        };
    };
...
// SPAWN PATROLS
    for "_counter" from 1 to _bGrps do {
        ...
        if (_bMin > 0) then {
            ...
            if (_debug) then {
                PLAYER SIDECHAT (format ["Spawned Patrol: %1",_counter]);0= [_mkr,_counter,"patrol",getpos (leader _bGroup)] call EOS_debug
            };
        };
    };
...
//SPAWN LIGHT VEHICLES
    for "_counter" from 1 to _cGrps do {
        ...
        if (_debug) then {
            player sidechat format ["Light Vehicle:%1 - r%2",_counter,_cGrps];0= [_mkr,_counter,"Light Veh",(getpos leader (_cGroup select 2))] call EOS_debug
        };
    };
...
//SPAWN ARMOURED VEHICLES
    for "_counter" from 1 to _dGrps do {
        ...
        if (_debug) then {
            player sidechat format ["Armoured:%1 - r%2",_counter,_dGrps];0= [_mkr,_counter,"Armour",(getpos leader (_dGroup select 2))] call EOS_debug
        };
    };
...
//SPAWN STATIC PLACEMENTS
    for "_counter" from 1 to _eGrps do {
        ...
        if (_debug) then {
            player sidechat format ["Static:%1",_counter];0= [_mkr,_counter,"Static",(getpos leader (_eGroup select 2))] call EOS_debug
        };
    };
...
//SPAWN CHOPPER
    for "_counter" from 1 to _fGrps do {
        ...
        if (_debug) then {
            player sidechat format ["Chopper:%1",_counter];0= [_mkr,_counter,"Chopper",(getpos leader (_fGroup select 2))] call EOS_debug
        };
    };
