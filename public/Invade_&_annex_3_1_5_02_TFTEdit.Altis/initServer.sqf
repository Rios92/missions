...
<t size='1'>We hope you have fun on our server. By playing here, you agree to abide by our server rules. In general: no teamkilling, don't goof off in base, and be respectful in chat. For full rules, check our website at www.ahoyworld.net.</t>", name _player], true, nil, 12, 0.3, 0.3] remoteExec ["BIS_fnc_textTiles", _player]; };
    ...
    {    _player assignCurator _zeusModule;
        ...
        [parseText format ["<br /><t align='center' font='PuristaBold' ><t size='1.6'>ZEUS (ADMIN) ASSIGNED</t><br />
<t size='1.2'>Welcome %1</t>", name _player], true, nil, 12, 0.3, 0.3] remoteExec ["BIS_fnc_textTiles", _player];
        _player setVariable ["zeusModule", _zeusModuleNumber];
        [_zeusModule,[-1,-2,0]] call BIS_fnc_setCuratorVisionModes;
    };
    
    if ( (zeusModeratorUIDs find _uid) > -1 ) then
    {   _player setVariable ["isZeus", true, true];
        diag_log format ['Moderator %1 connected. Zeus not assigned.', name _player];
        [parseText format ["<br /><t align='center' font='PuristaBold' ><t size='1.6'>ZEUS NOT ASSIGNED</t><br />
<t size='1.2'>Welcome %1 (Moderator)</t>", name _player], true, nil, 12, 0.3, 0.3] remoteExec ["BIS_fnc_textTiles", _player];
    };
    
    if ( (zeusSpartanUIDs find _uid) > -1 ) then
    {    _player assignCurator _zeusModule;
        [parseText format ["<br /><t align='center' font='PuristaBold' ><t size='1.6'>ZEUS (SPARTAN) ASSIGNED</t><br />
<t size='1.2'>Welcome %2</t>", name _player], true, nil, 12, 0.3, 0.3] remoteExec ["BIS_fnc_textTiles", _player];
