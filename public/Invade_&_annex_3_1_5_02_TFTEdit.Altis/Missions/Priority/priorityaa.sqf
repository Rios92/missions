...
Last modified:
	...
	16/03/2017 by Stan, AW community member
	
modified:

How stuff gets deleted after mission completion, added _groupsArray, groups get added to this array.
...
private ["_basepos","_loopVar","_dir","_PTdir","_pos","_barrier","_unitsArray","_flatPos","_accepted","_position","_enemiesArray","_targetList","_fuzzyPos","_x","_briefing","_enemiesArray","_unitsArray","_groupsArray","_flatPos1","_flatPos2","_flatPos3","_doTargets","_targetSelect","_targetListEnemy"];
...
//-------------------- 1. FIND POSITION FOR OBJECTIVE
	...
	while {!_accepted} do {
		...
		if ((_flatPos distance _basepos) > 2000) then {
			if ((_flatPos distance _basepos) < 15050) then {
				if ((_flatPos distance (getMarkerPos currentAO)) > 600) then {
					_accepted = true;
				};
			};
		};
...
//-------------------- 2. SPAWN OBJECTIVES
	...
	for "_m" from 1 to 99 do {
	priorityObj1 addMagazineTurret ["4Rnd_Titan_long_missiles",[0]]; priorityObj1 addMagazineTurret ["680Rnd_35mm_AA_shells_Tracer_Green",[0]]};
	...
	for "_m" from 1 to 99 do {
	priorityObj2 addMagazineTurret ["4Rnd_Titan_long_missiles",[0]]; priorityObj2 addMagazineTurret ["680Rnd_35mm_AA_shells_Tracer_Green",[0]]};
...
//-------------------- 6. THAT GIRL IS SO DANGEROUS!
	...
	[(units _priorityGroup)] call AW_FNC_AISkill;
...
{    _group = _x;{    deleteVehicle _x; } forEach (units _x);} forEach _groupsArray;
