...
{
	case "guns": {_info = [([] call VAS_Local_fnc_fetchPlayerGear),[1,2,4,5,4096]] call VAS_Local_fnc_filter;};
	case "mags": {_info = [([] call VAS_Local_fnc_fetchPlayerGear),"mag"] call VAS_Local_fnc_filter;};
	case "items": {_info = [([] call VAS_Local_fnc_fetchPlayerGear),"items"] call VAS_Local_fnc_filter;};
	case "packs": {_info = [([] call VAS_Local_fnc_fetchPlayerGear),"packs"] call VAS_Local_fnc_filter;};
	case "glass": {_info = [([] call VAS_Local_fnc_fetchPlayerGear),"glass"] call VAS_Local_fnc_filter;};
};

if(!isNil {(uiNamespace getVariable "VAS_UI_FILTER_VAR")}) then
{
	_info = [(uiNamespace getVariable "VAS_UI_FILTER_VAR"),true] call VAS_Local_fnc_filterMenu;
};
