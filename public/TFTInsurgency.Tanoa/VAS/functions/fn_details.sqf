...
if((uiNamespace getvariable "VAS_UI_FILTER") != "guns" && !(uiNamespace getVariable "VAS_UI_QuickMag")) exitWith {call VAS_Local_fnc_closeDetails;}; //Filter not on guns? exit
private["_control","_data","_control","_magazines","_details","_items","_addToLb"];
_control = _this select 0;
if(isNil {_control}) exitWith {call VAS_Local_fnc_closeDetails;};
if((_this select 1) == -1) exitWith {call VAS_Local_fnc_closeDetails;};
_data = (_control lbData (_this select 1));

_details = [_data] call VAS_Local_fnc_fetchCfgDetails;
if(count _details == 0) exitWith {call VAS_Local_fnc_closeDetails;}; //No info? Exit
if(!((_details select 4) in [1,2,4,5,4096])) exitWith {call VAS_Local_fnc_closeDetails;}; //Not a gun? Exit
if(count (_details select 7) == 0) exitwith {call VAS_Local_fnc_closeDetails;}; //No mags? Exit
...
_addToLb = {
	...
	_tmp = [_item,"CfgMagazines"] call VAS_Local_fnc_fetchCfgDetails;
...
_items = if (isNil "asdg_fnc_compatibleItems") then {[_data] call VAS_Local_fnc_accList} else {[_data] call asdg_fnc_compatibleItems};
...
if(count _items > 0) then {
	{
		_cfgInfo = [_x,"CfgWeapons"] call VAS_Local_fnc_fetchCfgDetails;
...
call VAS_Local_fnc_openDetails;