...
if (d_no_teamkill == 0 && {_dam >= 0.1 && {!isNull _injurer && {isPlayer _injurer && {_injurer != _unit && {isNull objectParent _unit && {side (group _injurer) == side (group _unit)}}}}}}) exitWith {
	if (_idx == -1 && {_ammo != "" && {time > ((player getVariable "d_tk_cutofft") + 3) && {getText (configFile>>"CfgAmmo">>_ammo>>"simulation") in __shots}}}) then {
...
if (_dam >= 0.9 && {time > _unit getVariable "d_last_leader_save"}) then {
	_unit setVariable ["d_last_leader_save", time + 2];
