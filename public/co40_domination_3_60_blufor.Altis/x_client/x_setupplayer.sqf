...
player setVariable ["d_last_leader_save", -1];
...
0 spawn {
	...
	if (d_player_can_call_arti > 0 || {d_player_can_call_drop > 0 || {d_string_player in d_can_call_cas || {d_with_ai || {d_with_ai_features == 0}}}}) then {
...
diag_log ["Internal D Version: 3.60"];
...
(findDisplay 46) displayAddEventHandler ["KeyDown", {
	if (param [1] in actionKeys "GetOver" &&  {alive player && {currentWeapon player == primaryWeapon player && {currentWeapon player != "" && {isNull objectParent player && {speed player > 11 && {stance player == "STAND" && {getFatigue player < 0.5 && {isTouchingGround (vehicle player) &&  {!(player getVariable "xr_pluncon") && {!d_p_isju}}}}}}}}}}) then {
		d_p_isju = true;
		0 spawn {
