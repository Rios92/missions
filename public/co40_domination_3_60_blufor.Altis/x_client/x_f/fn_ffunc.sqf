...
if (alive player && {!(player getVariable ["xr_pluncon", false]) && {isNull objectParent player}}) then {
	...
	if (_vUp select 2 < 0 && {player distance2D (player nearestObject d_rep_truck) < 20}) then {
		true
	} else {
		private _l = sqrt((_vUp select 0)^2 + (_vUp select 1)^2);
		if (_l != 0) then {
			(((_vUp select 2) atan2 _l) < 30 && {player distance2D (player nearestObject d_rep_truck) < 20})
		} else {
			false
		};
	};
