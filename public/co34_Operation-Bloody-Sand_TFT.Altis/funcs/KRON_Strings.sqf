...
FFS_StrLRSub = {
	private["_in","_lenL","_lenR","_arr","_i","_out"];
	_in=_this select 0;
	_lenL=_this select 1;
	_lenR=_this select 2;
	_arr=[_in] call KRON_StrToArray;
	_out="";
	if (_lenL>(count _arr)) then {_lenL=count _arr};
	for "_i" from (_lenL) to ((count _arr)-_lenR - 1) do {
