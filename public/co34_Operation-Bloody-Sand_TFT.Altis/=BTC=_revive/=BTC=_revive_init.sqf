...
BTC_who_can_revive  = FFS_who_can_revive;
BTC_disable_respawn = 0;
BTC_respawn_gear    = 1;
BTC_active_lifes    = FFS_active_lifes;
BTC_lifes           = FFS_lifes;
...
BTC_camera_unc      = 0;
BTC_camera_unc_type = ["Behind the back"];
BTC_respawn_time    = FFS_respawn_time;
BTC_active_mobile   = FFS_mobile_resp select 0;//Active mobile respawn (You have to put in map the vehicle and give it a name. Then you have to add one object per side to move to the mobile (BTC_base_flag_west,BTC_base_flag_east) - (1 = yes, 0 = no))
BTC_mobile_respawn  = FFS_mobile_resp select 0;//Active the mobile respawn fnc (1 = yes, 0 = no)
...
BTC_objects_actions_west = FFS_mobile_resp select 2;
BTC_objects_actions_east = [];
BTC_objects_actions_guer = [];
BTC_objects_actions_civ  = [];
if (isServer) then
{
	BTC_vehs_mobile_west = FFS_mobile_resp select 1;//Editable - define mobile west
