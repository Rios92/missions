...
{	
    ...
    if (_dead) then 
	{
		...
		if (_deadConfirm) then 
		{
			...
			if (_haveinit) then 
			{
				if (_unitinit == "heliInit") then 
				{
					_unit enableCopilot false;
					/*if (_type == "B_Heli_Transport_01_F" or _type == "B_Heli_Transport_01_camo_F") then 
					{
						_unit removeMagazinesTurret ["2000Rnd_65x39_Belt_Tracer_Red",[1]];
						_unit removeMagazinesTurret ["2000Rnd_65x39_Belt_Tracer_Red",[2]];
					};*/
				}else{
