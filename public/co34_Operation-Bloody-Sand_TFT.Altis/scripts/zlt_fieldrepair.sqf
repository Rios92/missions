...
if (isNil "zlt_cancelActionId") then {
	zlt_cancelActionId = player addAction["<t color='#FFA500'>"+STR_CANCEL_ACTION+"</t>", {zlt_mutexAction = false}, [], 20, false, true, '',' zlt_mutexAction  '];
	
	player addAction["<t color='#FFA500'>"+STR_FIELD_REPAIR+"</t>", zlt_prc_repairvehicle, [], 20, false, true, '','[] call zlt_fnc_repair_cond'];
	//player addAction["<t color='#FFA500'>"+STR_SERIOUS_REPAIR+ "</t>", zlt_fnc_heavyRepair, [], 20, false, true, '','_truck=(vehicle player);_truck getVariable ["zlt_repair_cargo", -1] != -1 and {alive cursorTarget} and {_truck distance cursorTarget <= 15} and {(cursorTarget isKindOf "LandVehicle" or cursorTarget isKindOf "Ship" or cursorTarget isKindOf "Air")} and {not zlt_mutexAction} and {speed cursorTarget <= 3} and {(damage cursorTarget != 0)}'];
	//player addAction["<t color='#FFA500'>"+STR_PUSH_APC+"</t>",zlt_pushapc,[],5,false,true,"","canMove (vehicle player) and ((vehicle player) isKindOf 'Wheeled_APC_F') and player == driver (vehicle player) and surfaceIsWater getpos (vehicle player)  and abs(speed (vehicle player) ) < 3 and not zlt_mutexAction"];   
...
player addEventHandler ["Respawn", {
	zlt_cancelActionId = player addAction["<t color='#0000ff'>"+STR_CANCEL_ACTION+"</t>", {zlt_mutexAction = false}, [], 20, false, true, '',' zlt_mutexAction  '];
	
	player addAction["<t color='#FFA500'>"+STR_FIELD_REPAIR+"</t>", zlt_prc_repairvehicle, [], 20, false, true, '','[] call zlt_fnc_repair_cond'];
	//player addAction["<t color='#FFA5000'>"+STR_SERIOUS_REPAIR+ "</t>", zlt_fnc_heavyRepair, [], 20, false, true, '','_truck=(vehicle player);_truck getVariable ["zlt_repair_cargo", -1] != -1 and {alive cursorTarget} and {_truck distance cursorTarget <= 15} and {(cursorTarget isKindOf "LandVehicle" or cursorTarget isKindOf "Ship" or cursorTarget isKindOf "Air")} and {not zlt_mutexAction} and {speed cursorTarget <= 3} and {(damage cursorTarget != 0)}'];
	//player addAction["<t color='#FFA500'>"+STR_PUSH_APC+"</t>",zlt_pushapc,[],5,false,true,"","canMove (vehicle player) and ((vehicle player) isKindOf 'Wheeled_APC_F') and player == driver (vehicle player) and surfaceIsWater getpos (vehicle player)  and abs(speed (vehicle player) ) < 3 and not zlt_mutexAction"];   
}];
