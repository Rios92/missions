...
if !(xr_cur_world_pos isEqualTo []) then {
	private _ne = xr_cur_world_pos nearEntities [["CAManBase", "Air", "Car", "Tank"], 3];
	private _ctrl = (uiNamespace getVariable "xr_SpectDlg") displayCtrl 5000;
	if !(_ne isEqualTo []) then {
		private _obj = crew (_ne select 0) select 0;
		if (isPlayer _obj) then {
			if (!ctrlShown _ctrl) then {
				_ctrl ctrlShow true;
			};
			if (xr_cur_world_obj != _obj) then {
				xr_cur_world_obj = _obj;
				private _name = [_obj] call d_fnc_gethpname;
				_ctrl ctrlSetText _name;
				_ctrl ctrlSetTextColor ([[1, 1, 1, 0.8], [0, 1, 0, 0.9]] select (group _obj == group player));
			};
			_ctrl ctrlSetPosition [xr_MouseCoord select 0, xr_MouseCoord select 1, 0.20, 0.06];
			_ctrl ctrlCommit 0;
		} else {
			if (count crew (_ne select 0) > 1) then {
				_obj = objNull;
				{
				...
				if (!isNull _obj) then {
					...
					_ctrl ctrlSetPosition [xr_MouseCoord select 0, xr_MouseCoord select 1, 0.10, 0.06];
					_ctrl ctrlCommit 0;
				};
			};
		};
...
if (!isNull xr_cur_world_obj && {xr_cur_world_obj != xr_spectcamtarget && {xr_MouseButtons select 0}}) then {
	private _lbctr = (uiNamespace getVariable "xr_SpectDlg") displayCtrl 1000;
