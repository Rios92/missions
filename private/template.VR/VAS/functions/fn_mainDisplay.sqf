...
[_request] call VAS_Local_fnc_filterShow;
...
{
	[(uiNamespace getVariable "VAS_UI_FILTER_VAR")] spawn VAS_Local_fnc_filterMenu;
};
uiNamespace setVariable ["VAS_UI_FILTER_VAR",nil];
uiNamespace setVariable ["VAS_UI_QuickMag",false];

[] call VAS_Local_fnc_updateLoad;
...
switch (_request) do
{
	case "guns":
	{
		...
		_info = ["guns"] call VAS_Local_fnc_fetchCfg;
		[_control,_info] spawn {
			...
			{
				_details = [_x,"CfgWeapons"] call VAS_Local_fnc_fetchCfgDetails;
		...
		_info = [([] call VAS_Local_fnc_fetchPlayerGear),[1,2,4,5,4096]] call VAS_Local_fnc_filter;
	...
	{
		_control = VAS_getControl(VAS_Main_Display,VAS_virt_list);
		_info = ["mags"] call VAS_Local_fnc_fetchCfg;
		[_control,_info] spawn {
			private["_control","_info"];
			disableSerialization;
			_control = _this select 0;
			_info = _this select 1;
			{
				_details = [_x,"CfgMagazines"] call VAS_Local_fnc_fetchCfgDetails;
				if(count _details > 0) then
				{
					_control lbAdd format["%1", (_details select 1)]; //Displayname on list
					_control lbSetData [(lbSize _control)-1,(_details select 0)]; //Data for index is classname
		...
		_control = VAS_getControl(VAS_Main_Display,VAS_unit_list);
		_info = [([] call VAS_Local_fnc_fetchPlayerGear),"mag"] call VAS_Local_fnc_filter;
		[_control,_info] spawn {
			private["_control","_info"];
			disableSerialization;
			_control = _this select 0;
	...
	{
		_control = VAS_getControl(VAS_Main_Display,VAS_virt_list);
		_info = ["items"] call VAS_Local_fnc_fetchCfg;
		[_control,_info] spawn {
			private["_control","_info"];
			disableSerialization;
			_control = _this select 0;
		...
		_control = VAS_getControl(VAS_Main_Display,VAS_unit_list);
		_info = [([] call VAS_Local_fnc_fetchPlayerGear),"items"] call VAS_Local_fnc_filter;
		[_control,_info] spawn {
			private["_control","_info"];
			disableSerialization;
			_control = _this select 0;
	...
	{
		_control = VAS_getControl(VAS_Main_Display,VAS_virt_list);
		_info = ["packs"] call VAS_Local_fnc_fetchCfg;
		[_control,_info] spawn {
			private["_control","_info"];
			disableSerialization;
			_control = _this select 0;
			_info = _this select 1;
			{
				_details = [_x,"CfgVehicles"] call VAS_Local_fnc_fetchCfgDetails;
				if(count _details > 0) then
				{
					_control lbAdd format["%1", (_details select 1)]; //Displayname on list
					_control lbSetData [(lbSize _control)-1,(_details select 0)]; //Data for index is classname
		...
		_control = VAS_getControl(VAS_Main_Display,VAS_unit_list);
		_info = [([] call VAS_Local_fnc_fetchPlayerGear),"packs"] call VAS_Local_fnc_filter;
	...
	{
		_control = VAS_getControl(VAS_Main_Display,VAS_virt_list);
		_info = ["glass"] call VAS_Local_fnc_fetchCfg;
		[_control,_info] spawn {
			private["_control","_info"];
			disableSerialization;
			_control = _this select 0;
			...
			{
				_details = [_x,"CfgGlasses"] call VAS_Local_fnc_fetchCfgDetails;
				if(count _details > 0) then
				{
					_control lbAdd format["%1", (_details select 1)]; //Displayname on list
					_control lbSetData [(lbSize _control)-1,(_details select 0)]; //Data for index is classname
		...
		_control = VAS_getControl(VAS_Main_Display,VAS_unit_list);
		_info = [([] call VAS_Local_fnc_fetchPlayerGear),"glass"] call VAS_Local_fnc_filter;
		[_control,_info] spawn {
			private["_control","_info"];
			disableSerialization;
			_control = _this select 0;
