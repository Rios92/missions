...
VAS_Local_fnc_quickMag =
{
	private["_data","_handle"];
	_data = (_this select 0) lbData (_this select 1);
	[_data,true,nil,nil,nil] call VAS_Local_fnc_handleItem;
	uiNamespace setVariable["VAS_UI_FILTER","mags"];
	uiNamespace setVariable ["VAS_UI_FILTER_VAR",nil];
	[] spawn VAS_Local_fnc_playerDisplay;
};

VAS_Local_fnc_quickItem =
{
	private["_data","_handle"];
	_data = (_this select 0) lbData (_this select 1);
	_handle = [_data,true,nil,nil,nil] spawn VAS_Local_fnc_handleItem;
	waitUntil {scriptDone _handle};
	[] spawn VAS_Local_fnc_playerDisplay;
};

VAS_Local_fnc_qRemoveItem =
{
	private["_data","_handle"];
	_data = (_this select 0) lbData (_this select 1);
	[_data,false,nil,nil,nil] call VAS_Local_fnc_handleItem;
	[] spawn VAS_Local_fnc_playerDisplay;
};